<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Data Komoditas</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola jenis tanaman/komoditas perkebunan.</p>
            </div>
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 border-y border-gray-200">
                    <tr>
                        <th class="px-6 py-4 font-bold tracking-wider w-16">No</th>
                            <th class="px-6 py-4 font-bold tracking-wider">Nama Komoditas</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-komoditas-{{ $k->id }}')" class="inline-flex items-center text-white font-medium text-xs px-3 py-1.5 bg-blue-500 hover:bg-blue-600 rounded shadow-sm transition mr-2">
                            ...
                            <x-modal name="edit-komoditas-{{ $k->id }}" focusable>
                                <form method="post" action="{{ route('komoditas.update', $k->id) }}" class="p-6 text-left whitespace-normal">
                                    @csrf
                                    @method('put')
                                    ...
                                    <div class="mt-6">
                                        <label for="nama_komoditas_{{ $k->id }}" class="block text-sm font-semibold text-gray-700 mb-1">Nama Komoditas</label>
                                        <input id="nama_komoditas_{{ $k->id }}" name="nama_komoditas" type="text" value="{{ $k->nama_komoditas }}" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                                    </div>
                            ...
                            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'hapus-komoditas-{{ $k->id }}')" class="inline-flex items-center text-white font-medium text-xs px-3 py-1.5 bg-red-500 hover:bg-red-600 rounded shadow-sm transition">
                                <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                Hapus
                            </button>

                            <!-- Modal Konfirmasi Hapus -->
                            <x-modal maxWidth="md" name="hapus-komoditas-{{ $k->id }}" focusable>
                                <form method="post" action="{{ route('komoditas.destroy', $k->id) }}" class="p-6 text-left whitespace-normal">
                                    @csrf
                                    @method('delete')
                                    ...
                                    <p class="mt-2 text-sm text-gray-600">
                                        Apakah Anda yakin ingin menghapus komoditas <strong>{{ $k->nama_komoditas }}</strong>? Data yang dihapus tidak dapat dikembalikan.
                                    </p>
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center">
                            ...
                            <h3 class="text-sm font-semibold text-gray-900">Belum ada komoditas</h3>
                            <p class="text-xs text-gray-500 mt-1">Silakan tambah data komoditas baru.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <x-modal name="tambah-komoditas" focusable>
        <form method="post" action="{{ route('komoditas.store') }}" class="p-6 text-left whitespace-normal">
            @csrf

            <h2 class="text-lg font-bold text-gray-900">
                {{ __('Tambah Komoditas Baru') }}
            </h2>

            <p class="mt-1 text-sm text-gray-600 mb-6">
                {{ __('Masukkan nama jenis tanaman atau komoditas perkebunan.') }}
            </p>

            <div class="mt-6">
                <label for="nama_komoditas" class="block text-sm font-semibold text-gray-700 mb-1">Nama Komoditas</label>
                <input id="nama_komoditas" name="nama_komoditas" type="text" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" placeholder="Contoh: Kelapa Sawit" required>
                <x-input-error :messages="$errors->get('nama_komoditas')" class="mt-2" />
            </div>
