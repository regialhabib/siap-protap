<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Data Wilayah & UPPT</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola data Unit Pelaksana Pelayanan Terpadu dan Kabupaten.</p>
            </div>
            <!-- Tombol Tambah yang memicu Modal -->
            <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'tambah-uppt')" class="inline-flex items-center justify-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg shadow-sm transition duration-150 whitespace-nowrap">
            ...
            <table class="w-full text-sm text-left whitespace-nowrap">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 border-y border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-bold tracking-wider">Nama UPPT</th>
                        <th class="px-6 py-4 font-bold tracking-wider">Kabupaten</th>
                        <th class="px-6 py-4 font-bold tracking-wider text-right w-32">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($uppts as $index => $u)
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4 text-gray-500">
                            {{ $uppts->firstItem() + $index }}
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-semibold text-gray-900">{{ $u->nama_uppt }}</span>
                        </td>
                        <td class="px-6 py-4 text-gray-500">
                            {{ $u->kabupaten->nama_kabupaten ?? '-' }}
                        </td>
                        <td class="px-6 py-4 text-right">
                            <!-- Edit Button (Triggers Modal) -->
                            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-uppt-{{ $u->id }}')" class="inline-flex items-center text-white font-medium text-xs px-3 py-1.5 bg-blue-500 hover:bg-blue-600 rounded shadow-sm transition mr-2">
                            ...
                            <x-modal name="edit-uppt-{{ $u->id }}" focusable>
                                <form method="post" action="{{ route('uppt.update', $u->id) }}" class="p-6 text-left">
                                    ...
                                    <div class="mt-6">
                                        <label for="nama_uppt_{{ $u->id }}" class="block text-sm font-semibold text-gray-700 mb-1">Nama UPPT</label>
                                        <input id="nama_uppt_{{ $u->id }}" name="nama_uppt" type="text" value="{{ $u->nama_uppt }}" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                                    </div>
                                    
                                    <div class="mt-4">
                                        <label for="kabupaten_id_{{ $u->id }}" class="block text-sm font-semibold text-gray-700 mb-1">Kabupaten</label>
                                        <select id="kabupaten_id_{{ $u->id }}" name="kabupaten_id" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                                            <option value="">-- Pilih Kabupaten --</option>
                                            @foreach($kabupatens as $kab)
                                                <option value="{{ $kab->id }}" {{ $u->kabupaten_id == $kab->id ? 'selected' : '' }}>{{ $kab->nama_kabupaten }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'hapus-uppt-{{ $u->id }}')" class="inline-flex items-center text-white font-medium text-xs px-3 py-1.5 bg-red-500 hover:bg-red-600 rounded shadow-sm transition">
                            ...
                            <x-modal name="hapus-uppt-{{ $u->id }}" focusable>
                                <form method="post" action="{{ route('uppt.destroy', $u->id) }}" class="p-6 text-left">
                                    ...
                                    <p class="mt-2 text-sm text-gray-600">
                                        Apakah Anda yakin ingin menghapus UPPT <strong>{{ $u->nama_uppt }}</strong>? Data yang dihapus tidak dapat dikembalikan.
                                    </p>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            ...
                            <h3 class="text-sm font-semibold text-gray-900">Belum ada data UPPT</h3>
                            <p class="text-xs text-gray-500 mt-1">Silakan tambah data UPPT baru.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <x-modal name="tambah-uppt" focusable>
        <form method="post" action="{{ route('uppt.store') }}" class="p-6">
            ...
            <p class="mt-1 text-sm text-gray-600 mb-6">
                Masukkan nama UPPT beserta Kabupaten yang menaunginya.
            </p>

            <div class="mt-6">
                <label for="nama_uppt" class="block text-sm font-semibold text-gray-700 mb-1">Nama UPPT</label>
                <input id="nama_uppt" name="nama_uppt" type="text" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" placeholder="Contoh: UPPT Muaro Jambi" required>
                <x-input-error :messages="$errors->get('nama_uppt')" class="mt-2" />
            </div>
            
            <div class="mt-4">
                <label for="kabupaten_id" class="block text-sm font-semibold text-gray-700 mb-1">Kabupaten</label>
                <select id="kabupaten_id" name="kabupaten_id" class="block w-full px-4 py-3 rounded-md border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" required>
                    <option value="">-- Pilih Kabupaten --</option>
                    @foreach($kabupatens as $kab)
