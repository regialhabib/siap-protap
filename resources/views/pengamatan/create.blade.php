<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-12">
            ...
            <form method="POST" action="{{ route('pengamatan.store') }}" class="p-6 md:p-10" x-data="{
                ringan: {{ old('serangan_ringan', 0) }},
                sedang: {{ old('serangan_sedang', 0) }},
                berat: {{ old('serangan_berat', 0) }},
                get totalSerangan() {
                    ...
                    <div class="flex flex-col gap-6 bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div>
                            ...
                            <select name="opt_id" class="block w-full px-4 py-3 rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm transition-colors cursor-pointer" required>
                                ...
                                @foreach($opts as $o)
                                    <option value="{{ $o->id }}" {{ old('opt_id') == $o->id ? 'selected' : '' }}>{{ $o->nama_opt }}</option>
                                @endforeach
                            </select>
                ...
                <div class="mb-12">
                    ...
                    <div class="mb-6 bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div class="relative w-full">
                            <input type="number" step="0.01" min="0" name="luas_komoditi_ha" value="{{ old('luas_komoditi_ha') }}" class="block w-full pl-5 pr-14 py-3.5 rounded-xl border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-lg font-bold text-gray-900 transition-colors" placeholder="0.00" required>
                ...
                <div class="mb-10">
                    ...
                    <div class="bg-blue-50/30 p-6 md:p-8 rounded-xl border border-blue-100 mb-6 relative overflow-hidden">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10">
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="kendali_apbd_kab" value="{{ old('kendali_apbd_kab') }}" class="block w-full px-4 py-2.5 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium transition-colors" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-wider">APBD Provinsi</label>
                                <input type="number" step="0.01" min="0" name="kendali_apbd_prov" value="{{ old('kendali_apbd_prov') }}" class="block w-full px-4 py-2.5 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium transition-colors" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-wider">Swadaya/Masyarakat</label>
                                <input type="number" step="0.01" min="0" name="kendali_masyarakat" value="{{ old('kendali_masyarakat') }}" class="block w-full px-4 py-2.5 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium transition-colors" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-wider">APBN Pusat</label>
                                <input type="number" step="0.01" min="0" name="kendali_apbn" value="{{ old('kendali_apbn') }}" class="block w-full px-4 py-2.5 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium transition-colors" placeholder="0.00">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50/50 p-6 md:p-8 rounded-xl border border-gray-100">
                        ...
                        <input type="text" name="kondisi_serangan" value="{{ old('kondisi_serangan') }}" class="block w-full px-4 py-3 rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm transition-colors" placeholder="Contoh: Terkendali, Aman, Waspada, Eksplosif...">
                    </div>
                </div>
